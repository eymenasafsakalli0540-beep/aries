...
def get_logs():
    ...
    # 🗑️ PANELDEKİ SİLME TUŞUNA BASILDIYSA DOSYAYI KÖKTEN SİL VE SIFIRLA
    if action == 'clear':
        try:
            if os.path.exists("sorular.txt"):
                os.remove("sorular.txt")  # Dosyayı sunucudan tamamen kaldırıyoruz
...
def ask():
    ...
    if any(x in norm_msg for x in ["kim yapti", "yapimcin", "kim gelistirdi", "kurucun", "sahibin", "sen kimsin", "adini kim verdi"]):
        save_log("CEVAPLANDI")
        if is_buddy_mode:
            return jsonify({"reply": '<span class="expert-badge badge-sozel">Sistem Çekirdeği</span><br>Beni tam bir dahi olmam için <b>TÜW</b> geliştirdi kanka! Adım <b>ARIES AI</b>, yaratıcım and tek liderim <b>TÜW</b>\'dür. 🚀'})
